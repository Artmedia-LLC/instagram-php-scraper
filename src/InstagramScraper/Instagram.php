...
class Instagram
{
     ...
     * @return void
     */
    public function getMediasByUsername($username, $count){
        $accounts = Instagram::searchAccountsByUsername($username, 1);
        ...
        return Instagram::getMediasByUserId($userId, $count,'');
    }
/**
    ...
    */
    ...
    public function getMediasByUserId($id, $count = 12, $maxId = '')
    {
        ...
        while ($index < $count && $isMoreAvailable) {
            ...
            //dd($arr['data']['user']['edge_owner_to_timeline_media']['edges']);
            if (!is_array($arr)) {
                throw new InstagramException('Response code is ' . $response->code . '. Body: ' . static::getErrorBody($response->body) . ' Something went wrong. Please report issue.', $response->code);
            }
...
//        return md5(implode(':', [$this->getRhxGis(), $variables]));
     ...
     */
    public function getMediasByTag($tag, $count = 12, $type = 'edge_hashtag_to_media', $minTimestamp = null)
    {
        $index = 0;
        $medias = [];
        $mediaIds = [];
        ...
        while ($index < $count && $hasNextPage) {
            $response = Request::get(Endpoints::getMediasJsonByTagLink($tag), $this->generateHeaders($this->userSession));
            if ($response->code === static::HTTP_NOT_FOUND) {
...
//            dd($arr['graphql']['hashtag']['edge_hashtag_to_media']['edges']);
//            dd($arr['graphql']['hashtag']['edge_hashtag_to_top_posts']);
            ...
            if (empty($arr[$rootKey]['hashtag']['edge_hashtag_to_media'])) {
                return [];
            }
...
//                dd($item['node']);
     ...
     * @throws \Psr\Http\Client\ClientExceptionInterface
     */
    public function getFirstMediasByTag($tag, $count = 12, $type = 'top', $maxId = '', $minTimestamp = null)
    {
        $index = 0;
        $medias = [];
        $mediaIds = [];
        ...
        while ($index < $count && $hasNextPage) {
            ...
            $rootKey = array_key_exists('graphql', $arr) ? 'graphql' : 'data';
            if (empty($arr[$rootKey][$type]["sections"])) {
                return [];
            }
            $nodes = $arr[$rootKey][$type]["sections"];
            foreach ($nodes as $mediaArray) {
                $mediaList = $mediaArray["layout_content"]["medias"];
                foreach ($mediaList as $item) {
                    if ($index === $count) {
                        return $medias;
                    }

                    if (isset($item['media']['carousel_media'])){
                        $media = Media::create($item['media']['carousel_media'][0]);
                    }else{
                        $media = Media::create($item['media']);
                    }
     ...
     */
    public function login($force = true, $twoStepVerificator = null)
    {
...
//                    dd('ss');
                    ...
                    throw new InstagramAuthException('Something went wrong. Please report issue. ' . strval($response->code), $response->code );
                }
            }
